...
func main() {
	...
	r.PathPrefix("/").Handler(http.FileServer(http.Dir("./webauthn")))
...
func BeginLogin(w http.ResponseWriter, r *http.Request) {
	...
	loginOption := func(cd *protocol.PublicKeyCredentialRequestOptions) {
		cd.UserVerification = protocol.VerificationDiscouraged
	}

	// generate PublicKeyCredentialRequestOptions, session data
	options, sessionData, err := webAuthn.BeginLogin(user, loginOption)
	if err != nil {
		log.Println(err)
		jsonResponse(w, err.Error(), http.StatusInternalServerError)
		return
