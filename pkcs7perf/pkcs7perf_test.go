...
func TestDegenerateCertificate(t *testing.T) {
	...
	deg, err := pkcs7.DegenerateCertificate(cert.Certificate.Raw)
	if err != nil {
		t.Fatal(err)
	}
...
func ExampleSignedData() {
	...
	signedData, err := pkcs7.NewSignedData([]byte("Example data to be signed"))
	if err != nil {
...
func TestOpenSSLVerifyDetachedSignature(t *testing.T) {
	...
	if err := toBeSigned.AddSigner(signerCert.Certificate, signerCert.PrivateKey, pkcs7.SignerInfoConfig{}); err != nil {
		t.Fatalf("Cannot add signer: %s", err)
	}
	toBeSigned.Detach()
	signed, err := toBeSigned.Finish()
...
func TestEncrypt(t *testing.T) {
	...
	for _, mode := range modes {
		pkcs7.ContentEncryptionAlgorithm = mode
		...
		encrypted, err := pkcs7.Encrypt(plaintext, []*x509.Certificate{cert.Certificate})
		if err != nil {
			t.Fatal(err)
		}
...
func TestUnmarshalSignedAttribute(t *testing.T) {
	...
	testValue := "TestValue"
	if err := toBeSigned.AddSigner(cert.Certificate, cert.PrivateKey, pkcs7.SignerInfoConfig{
		ExtraSignedAttributes: []pkcs7.Attribute{pkcs7.Attribute{Type: oidTest, Value: testValue}},
	}); err != nil {
		t.Fatalf("Cannot add signer: %s", err)
	}
	signed, err := toBeSigned.Finish()
